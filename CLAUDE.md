...
- [ ] **Seguimiento de Siembra por lote** — migraciones `Docs/planting_lot_tracking.sql` y `Docs/planting_multi_bed.sql` aplicadas en Supabase. Un lote puede abarcar múltiples camas mediante `planting_beds`, mantiene un único `Lot ID` y conserva `plantings.bed_id` como primera cama por compatibilidad. Seguimiento, cosecha, Aplicar Insumos y Mantenimiento encuentran el mismo lote desde cualquiera de sus camas. `Seguimiento de Siembra` tiene conteo de establecimiento único (objetivo por defecto a +21 días, editable al crear por el trabajador y luego por admin), incidencias/seguimientos opcionales y cierre explícito (`closed`/`lost`). Los trabajadores usan formularios y selectores Área → Subárea → Cama → lote activo (cultivo + Lot ID), mientras que el detalle, historial y métricas son admin-only. Aplicar Insumos y Mantenimiento detectan automáticamente los lotes activos de los targets registrados; costos por lote son estimados con reparto igual inicial, nunca presentados como exactos. Pendiente prueba end-to-end con login real.
