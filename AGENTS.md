...
- [x] **Primer slice del sistema de tareas asignadas implementado en código** — nuevo módulo frontend `js/modules/tasks.js` + API `/api/tasks` en `worker/worker.js`; admins pueden crear tareas con módulo, formulario, fecha límite y recurrencia (`none` / `weekly` / `biweekly` / `monthly`), usuarios no-admin ven sus pendientes en el dashboard, y al completar el formulario asignado la tarea se marca como completada automáticamente y genera la siguiente recurrencia si aplica. La migración SQL quedó en `Docs/task_assignments.sql` y las utilidades/validaciones en `worker/task-utils.mjs` con regresión en `tests/task-utils.test.mjs`.
...
- [ ] **Aplicar en Supabase la migración `Docs/task_assignments.sql`** — requisito para que el primer slice de tareas asignadas funcione fuera del código local.
